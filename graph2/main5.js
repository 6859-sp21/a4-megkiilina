...
//------filtering & creating datasets for each chart------//
  ...
  // draw chart based on the button
    ...
    /* Add line into SVG */
      var line = d3.line()
        .x(d => xScale(d.xlrealgdp))
        .y(d => yScale(d.cellphone));
      ...
      lines.selectAll("circle-group")
        ...
        .on("mouseover", function(d) {
            d3.select(this)     
              ...
              .attr("class", "text")
              .text(`${d.cellphone}`)
              .attr("x", d => xScale(d.xlrealgdp) + 5)
              .attr("y", d => yScale(d.cellphone) - 10);
          })
        .on("mouseout", function(d) {
            d3.select(this)
              .style("cursor", "none")  
        ...
        .append("circle")
        .attr("cx", d => xScale(d.xlrealgdp))
        .attr("cy", d => yScale(d.cellphone))
        .attr("r", circleRadius)
        .style('opacity', circleOpacity)
        .on("mouseover", function(d) {
              d3.select(this)
